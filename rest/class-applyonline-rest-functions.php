    ...
    function __construct() {
        ...
        function file_uploader($errors, $post, $files){
            ...
            foreach($files as $key => $val):
                ...
                if( !in_array($file_ext, $allowed_types) ) $errors->add(406, sprintf(esc_html__( 'Invalid file %1$s. Allowed file types are: %2$s', 'apply-online' ), $val['name'], implode (',', $allowed_types)));
                ...
                if(empty($errors->errors)){
                    ...
                    if ( $movefile && ! isset( $movefile['error'] ) ) {
                         ...
                         */
                         $errors->add(400, $val['name'].': '.$movefile['error']);
                    }
                }
            endforeach;
        ...
        public function form_post( WP_REST_REQUEST $request ){
            ...
            foreach( $transcript as $key => $val ):
                ...
                if( !(isset($form_data[$key]) OR isset($_FILES[$key])) ) $errors->add(406, esc_html__("Some fields are missing.", 'apply-online') );
                ...
                //eMail validation.
                if( $val['type'] == 'email'){
                    if( !empty($form_data[$key]) and !is_email($form_data[$key]) ) $errors->add(406, sprintf(esc_html__('%s is invalid.', 'apply-online'), '"'.$val['label'].'"'));
                }
            endforeach;
            ...
            if( !empty( $error_messages ) ){
                ...
                return new WP_REST_Response( $response, $errors->get_error_code() );
            }
